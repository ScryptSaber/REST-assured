...
public class ECommerceAPITest {
    public static void main(String[] args) {
        ...
        logreq.setUserEmail("aerdemkamis@gmail.com");
        logreq.setUserPassword("Erdem123456");

        RequestSpecification reqLogin = given().relaxedHTTPSValidation().log().all().spec(req).body(logreq);
        LoginResponse logResponse = reqLogin.when().post("/api/ecom/auth/login").then().log().all().extract().response().as(LoginResponse.class);

        System.out.println(logResponse.getToken());
        String token = logResponse.getToken();
        System.out.println(logResponse.getUserId());
        ...
        RequestSpecification reqAddProduct = given().log().all().spec(addProductBaseReq)
                ...
                .multiPart("productImage", new File("C:\\Users\\ASUS\\Desktop\\laptop.png"));
